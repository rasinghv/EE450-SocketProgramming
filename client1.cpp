...
int main(int argc, char *argv[]){
	...
	string line;
  	...
  	if(mySearchterms.find(sTerm) == mySearchterms.end()){
  		...
  		cout << "The Client 1 has received a request with search word " << sTerm << ", which maps to key " << searchKey << "." << endl;
  	}
	...
	cout << "The Client 1 sends the request " << myRequestString << " to the Server 1 with port number " << ntohs(their_addr.sin_port);
	cout << " and IP address " << inet_ntoa(their_addr.sin_addr) << "." << endl;

	cout << "The Client1’s port number is " << ntohs(mysin.sin_port);
	...
	cout << "The Client 1 received the value " << buf << " from the Server 1 with port number " << ntohs(their_addr.sin_port);
	cout << " and IP address " << inet_ntoa(their_addr.sin_addr) << "." << endl;
